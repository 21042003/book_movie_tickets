...
import 'package:book_movie_tickets/features/seat_selection/repository/showtime_repository.dart';
import 'package:book_movie_tickets/core/widgets/success_dialog.dart';
...
class _QRPaymentScreenState extends ConsumerState<QRPaymentScreen> {
  ...
  void _simulateSuccess() async {
    ...
    // Save to local SQLite database only when payment is "successful"
    try {
        ...
        showDialog(
          ...
          if (mounted) {
            Navigator.pushReplacement(
              ...
              MaterialPageRoute(
                builder: (context) =>
                    MyTicketScreen(booking: widget.booking, movie: widget.movie),
              ),
            );
          }
        });
  ...
  Future<void> _handleBack() async {
    ...
    final bool? confirmCancel = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          tr.confirmCancelPayment,
          style: const TextStyle(color: Colors.white70),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: Text(tr.no, style: const TextStyle(color: Colors.grey)),
          ),
          TextButton(
    ...
    if (confirmCancel == true) {
      if (mounted) {
        ...
        try {
          await ref
              ...
              .cancelBooking(
                ...
                seatLabels: widget.booking.seats,
              );
        } catch (e) {
        ...
        if (mounted) {
          ...
          Navigator.pop(context); // Pop QRPaymentScreen
        }
      }
    }
  }
